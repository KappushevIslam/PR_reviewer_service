...
from app.schemas.pull_request import (
    ...
    PullRequestReassign, PullRequestShort, UserReviewList
)
    ...
    if not db_pr:
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": ErrorCode.NOT_FOUND,
                    "message": f"PR '{reassign_data.pull_request_id}' not found"
                }
            }
        )
    ...
    if not old_reviewer:
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": ErrorCode.NOT_FOUND,
                    "message": f"Reviewer '{reassign_data.old_user_id}' not found"
                }
            }
        )
    ...
    if not team:
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": ErrorCode.NOT_FOUND,
                    "message": f"Team '{old_reviewer.team_name}' not found"
                }
            }
        )
